...
/**
 * Controlador que se encarga de mostrar los datos basados a un filtrado
 * indicado en los distintos métodos
 * 
...
public class ControllerPENDIENTESandTERMINADAS {
	...
	void initialize() {
		...
		// la tabla se carga con los valores de la general filtrados por este filtro
		// según necesidad
		tablaPendiID.setItems(cargarDatos());
		okk = true;
		// listener de ratón si usuario hace click sobre un campo de la tabla abre el
		// modal
		tablaPendiID.setOnMouseClicked(event -> mostrarVentana(cita));
		// listener de selección si el usuario selecciona un elemento de la tabla se le
		// da ese valor a la cita
		tablaPendiID.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
			cita = newValue;
		});
	...
	/**
	 * Método que se encarga de actualizar la tabla comprobando en que boton fue
	 * pulsado si borrar o editar, si es EDITAR comprueba que tabla esta
	 * seleccionada si acabadas o pendientes, y comprueba en cada caso si la fecha
	 * ha sido modificada si la fecha en acabadas es modificada a una fecha mayor
	 * esta sera borrada de la tabla y se vuelve a cargar los datos, en el caso
	 * contrario pasa lo mismo pero si la fecha es superior
	 * 
	...
	/**
	 * Metodo cargar datos filtrando los datos de la general que tiene un observable
	 * que será adjudicado a la tabla con el filtro correspondiente donde según que
	 * pestaña este el usuario carga unos u otros para ser mostrados esto devuelve
	 * una lista, si no hay elementos que cumplan las condiciones devuelve la tabla
	 * vacía
	 * 
	...
	/**
	 * método llamado desde listener para que abra el modal si hay una celsa
	 * seleccionada
	 * 
	...
	private void mostrarVentana(Citas cita) {
		...
		try {
			// Cargue el archivo fxml y cree una nueva etapa para el cuadro de diálogo
			// emergente.
