...
public class PracticaMain extends Application {
	...
	public void iniciPrincipal() {
		try {
			...
			/*
			 * Manejador de ratï¿½n sobre la escena iniciar sesiï¿½n muestra el valor de
			 * donde pulsa
			 */
	...
	public void abrirGeneral() {
		try {
			...
			/**
			 * Eventos de teclado creados en la escena general para poder acceder a dos
			 * ventanas de la aplicación según donde mantenga pulsado
			 */
			scene.setOnKeyReleased((KeyEvent keyEvent) -> {
				...
				if (keyEvent.isControlDown()) {
					...
					} else if ((keyEvent.getCode().toString().equals("B")
							|| keyEvent.getCode().toString().equals("b"))) {
