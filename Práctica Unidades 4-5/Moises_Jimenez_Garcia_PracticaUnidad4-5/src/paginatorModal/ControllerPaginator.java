...
public class ControllerPaginator {
	...
	@FXML
	/*
	 * Clase adaptada de un ejercicio visto en clase una vez entendida, para añadir
	 * una forma de avisar al usuario de los atajos que encontrará en la aplicación
	 * antes de usarla
	 */
	void initialize() {
		...
		paginator.setPageCount((atajos.size() / limite) + (((atajos.size() % limite) > 0) ? 1 : 0));
		...
		paginator.currentPageIndexProperty().addListener((observable, oldValue, newValue) -> {
			progresoIndicado.setProgress((newValue.doubleValue() + 1) / paginator.getPageCount());
		});
