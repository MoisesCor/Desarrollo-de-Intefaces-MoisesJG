...
public class ControllerTutorial {
	...
	private final Node rootIcon = new ImageView(new Image(getClass().getResourceAsStream("icon1.JPG")));
	private final Node rootIcon2 = new ImageView(new Image(getClass().getResourceAsStream("icon1.JPG")));
	private final Node rootIcon3 = new ImageView(new Image(getClass().getResourceAsStream("icon1.JPG")));

	@FXML
	void initialize() {
		TreeItem<String> rootItem = new TreeItem<String>("Datos", rootIcon); // aquí en el cosntructor le metes la
																				// imagen
		...
		lista.getItems().addAll("Recomendarías la aplicación", "Te facilita el trabajo", "Funciona siempre bien",
				"Es facil de usar", "Te gusta el diseño");

		// lisener para obtener el nuevo valor
		lista.getSelectionModel().selectedItemProperty()
				.addListener((observable, oldValue, newValue) -> resultado = newValue);
	...
	void mostrarDialogo(MouseEvent event) {
		...
		// lisener que nos recoje la posición del actual selección de la lista, y
		// cambiamos el valor por lo elegido
